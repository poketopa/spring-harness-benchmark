...
public class ReservationService {
    ...
    public ReservationWaitingResponse createWaiting(LoginMember loginMember, ReservationRequest request) {
        ...
        Reservation reservation = reservationRepository.findByThemeAndTimeAndDate(theme, time, request.date())
                .orElseThrow(() -> new RoomescapeException(
                        ErrorCode.WAITING_SLOT_AVAILABLE,
                        "예약 가능한 시간에는 대기를 신청할 수 없습니다."
                ));
        if (reservation.isOwnedBy(member)) {
            throw new RoomescapeException(ErrorCode.WAITING_OWN_RESERVATION, "본인의 예약에는 대기를 신청할 수 없습니다.");
        }
        ...
        try {
            ...
            return ReservationWaitingResponse.of(saved, calculateWaitingRank(saved));
        } catch (DataIntegrityViolationException exception) {
            throw new RoomescapeException(ErrorCode.DUPLICATE_WAITING, "이미 대기 신청한 시간입니다.");
        }
    }
    ...
    public List<MyReservationResponse> findMine(LoginMember loginMember) {
        ...
        reservationRepository.findAllByMemberOrderByDateAscTimeStartAtAsc(member)
                .stream()
                .map(MyReservationResponse::fromReservation)
                .forEach(responses::add);
        reservationWaitingRepository.findAllByMemberOrderByDateAscTimeStartAtAscCreatedAtAscIdAsc(member)
                .stream()
                .map(waiting -> MyReservationResponse.fromWaiting(waiting, calculateWaitingRank(waiting)))
                .forEach(responses::add);
    ...
    private int calculateWaitingRank(ReservationWaiting waiting) {
        long ahead = reservationWaitingRepository.countAhead(
                waiting.getTheme(),
                waiting.getTime(),
                waiting.getDate(),
