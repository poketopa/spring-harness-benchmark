...
class ReservationAcceptanceTest extends AcceptanceTestSupport {
    ...
    @Test
    @DisplayName("이미 예약된 슬롯을 예약하면 대기 예약을 생성하고 대기 순번을 반환한다")
    void occupiedReservationCreatesWaitingWithRank() {
        ...
        assertThat(waiting.getBody().get("waitingRank")).isEqualTo(1);
    ...
    @Test
    @DisplayName("예약 대기는 먼저 요청한 사용자부터 순번을 부여한다")
    void waitingRankIncreasesByRequestOrder() {
        createMember("브라운", "brown@example.com");
        createMember("코니", "cony@example.com");
