...
public class AuthTokenProvider {
    ...
    public String createToken(Member member, String loginTokenId) {
        String payload = PREFIX + member.getId() + DELIMITER + loginTokenId;
        return Base64.getUrlEncoder()
                .withoutPadding()
                .encodeToString(payload.getBytes(StandardCharsets.UTF_8));
    }

    public AccessTokenPayload extractPayload(String token) {
        try {
            ...
            return new AccessTokenPayload(Long.parseLong(values[0]), values[1]);
