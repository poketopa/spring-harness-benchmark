...
public class ReservationTimeService {
    ...
    public ReservationTimeResponse create(ReservationTimeRequest request) {
        ReservationTime time = reservationTimeRepository.save(new ReservationTime(
                storeRepository.getDefaultStore(),
                request.startAt()
        ));
    ...
    public List<ReservationTimeResponse> findThemeTimes(Long themeId, LocalDate date) {
        ...
        return findTimesForThemeStore(theme)
                .stream()
                .map(time -> ReservationTimeResponse.of(time, reservedTimeIds.contains(time.getId())))
                .toList();
    }

    private List<ReservationTime> findTimesForThemeStore(Theme theme) {
