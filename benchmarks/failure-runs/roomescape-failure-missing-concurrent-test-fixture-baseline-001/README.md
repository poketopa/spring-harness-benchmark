# Roomescape JPA Auth Concurrent Login Regen V3
...
- Simultaneous first login requests for the same account do not leak database uniqueness failures as server errors.
...
- `AuthService.login(...)` locks the target member row while creating or renewing `AuthSession`.
- `AuthService.login(...)` uses `saveAndFlush(...)` so session persistence conflicts surface inside the service transaction.
...
- `AuthServiceTest` covers token renewal, stale-token rejection, and returned login member identity.
- `AuthSessionRepositoryIntegrationTest` covers active-session lookup and one-session-per-member persistence constraint.
- `ConcurrentLoginAcceptanceTest` covers sequential token invalidation and simultaneous first login requests.
