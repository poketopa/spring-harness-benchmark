...
public class WaitingService {
    ...
    public WaitingResponse create(LoginMember loginMember, WaitingRequest request) {
        ...
        validateCreateAllowed(member, reservation);

        Waiting waiting = new Waiting(member, reservation, LocalDateTime.now(clock));
        try {
            Waiting saved = waitingRepository.saveAndFlush(waiting);
            return WaitingResponse.of(saved, waitingRepository.rankOf(saved.getReservation(), saved.getCreatedAt(), saved.getId()));
    ...
    private Reservation findReservedSlot(Theme theme, ReservationTime time, WaitingRequest request) {
        return reservationRepository.findByThemeAndTimeAndDate(theme, time, request.date())
                .orElseThrow(() -> new RoomescapeException(ErrorCode.AVAILABLE_SLOT_WAITING, "예약되지 않은 슬롯에는 대기할 수 없습니다."));
    }

    private void validateCreateAllowed(Member member, Reservation reservation) {
