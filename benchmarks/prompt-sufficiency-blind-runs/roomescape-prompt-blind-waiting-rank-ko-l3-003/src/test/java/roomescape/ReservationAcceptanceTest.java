...
class ReservationAcceptanceTest extends AcceptanceTestSupport {
    ...
    @Test
    @DisplayName("예약이 꽉 찬 시간에는 대기를 신청하고 대기 순번을 확인할 수 있다")
    void fullReservationCreatesWaitingWithRank() {
        ...
        assertThat(firstWaiting.getBody().get("waiting")).isEqualTo(true);
        assertThat(firstWaiting.getBody().get("waitingRank")).isEqualTo(1);
        assertThat(secondWaiting.getStatusCode()).isEqualTo(HttpStatus.CREATED);
        assertThat(secondWaiting.getBody().get("waiting")).isEqualTo(true);
        assertThat(secondWaiting.getBody().get("waitingRank")).isEqualTo(2);

        List<Map<String, Object>> mine = findMine(sallyToken);
        assertThat(mine).hasSize(1);
        assertThat(mine.getFirst().get("waiting")).isEqualTo(true);
        assertThat(mine.getFirst().get("waitingRank")).isEqualTo(2);
    }

    @Test
    @DisplayName("같은 시간에 중복 대기를 신청하면 충돌 응답을 반환한다")
    ...
    @Test
    @DisplayName("이미 예약한 회원이 같은 슬롯을 다시 예약하면 충돌 응답을 반환한다")
    void duplicateOwnReservationReturnsConflict() {
        createMember("브라운", "brown@example.com");
        String token = login("brown@example.com");
        long themeId = createTheme("어둠의 방");
        long timeId = createTime(LocalTime.of(10, 0));
        ...
        ResponseEntity<Map> duplicate = createReservation(token, LocalDate.of(2030, 5, 1), timeId, themeId);
