...
class ReservationAcceptanceTest extends AcceptanceTestSupport {
    ...
    @Test
    @DisplayName("회원은 본인의 예약 날짜와 시간을 변경할 수 있다")
    void memberChangesOwnReservationDateAndTime() {
        createMember("브라운", "brown@example.com");
        String token = login("brown@example.com");
        long themeId = createTheme("어둠의 방");
        ...
        long reservationId = ((Number) createReservation(token, originalDate, tenOClockId, themeId)
                .getBody()
                .get("id")).longValue();

        ResponseEntity<Map> response = changeReservation(token, reservationId, changedDate, elevenOClockId);
    ...
    void changeReservationToOccupiedSlotReturnsConflict() {
        ...
        ResponseEntity<Map> response = changeReservation(brownToken, brownReservationId, date, elevenOClockId);
