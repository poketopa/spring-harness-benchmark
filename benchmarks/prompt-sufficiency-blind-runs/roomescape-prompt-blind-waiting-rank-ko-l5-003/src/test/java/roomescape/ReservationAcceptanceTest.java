...
class ReservationAcceptanceTest extends AcceptanceTestSupport {
    ...
    @Test
    @DisplayName("이미 예약된 슬롯에는 대기 신청할 수 있다")
    void waitingCanBeCreatedForOccupiedSlot() {
        createMember("브라운", "brown@example.com");
    ...
    @Test
    @DisplayName("예약되지 않은 슬롯에는 대기 신청할 수 없다")
    void availableSlotWaitingReturnsBadRequest() {
        ...
        ResponseEntity<Map> waiting = createWaiting(token, LocalDate.of(2030, 5, 1), timeId, themeId);

        assertThat(waiting.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
        assertThat(waiting.getBody().get("code")).isEqualTo("WAITING_SLOT_NOT_RESERVED");
    }

    @Test
    @DisplayName("본인의 예약에는 대기 신청할 수 없다")
    ...
    void mineContainsReservationsAndWaitingsWithRank() {
        createMember("브라운", "brown@example.com");
        createMember("코니", "cony@example.com");
        createMember("샐리", "sally@example.com");
        String brownToken = login("brown@example.com");
