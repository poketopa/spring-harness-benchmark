...
public class ReservationService {
    ...
    public ReservationResponse createWaiting(LoginMember loginMember, ReservationRequest request) {
        ...
        Reservation occupiedReservation = reservationRepository.findByThemeAndTimeAndDate(theme, time, request.date())
                .orElseThrow(() -> new RoomescapeException(ErrorCode.WAITING_NOT_ALLOWED, "예약 가능한 시간은 대기할 수 없습니다."));
        if (occupiedReservation.isOwnedBy(member)) {
        ...
        try {
            ...
            return ReservationResponse.of(saved, rankOf(saved));
        } catch (DataIntegrityViolationException exception) {
            throw new RoomescapeException(ErrorCode.DUPLICATE_WAITING, "이미 대기 중인 시간입니다.");
        }
    }
    ...
    public List<ReservationResponse> findMine(LoginMember loginMember) {
        ...
        List<ReservationResponse> reservations = reservationRepository.findAllByMemberOrderByDateAscTimeStartAtAsc(member)
                ...
                .toList();
        List<ReservationResponse> waitings = waitingRepository.findAllByMemberOrderByDateAscTimeStartAtAscCreatedAtAscIdAsc(member)
                .stream()
                .map(waiting -> ReservationResponse.of(waiting, rankOf(waiting)))
                .toList();
