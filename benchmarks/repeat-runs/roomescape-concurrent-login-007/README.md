...
- New login wins.
- When the same member logs in again, the previous token becomes inactive.
- Token authentication checks both token payload validity and server-side active session state.

### Design Decisions

- `AuthSession` stores one active session key per member.
- `AuthService.login(...)` locks the member row before renewing the active session so simultaneous same-member logins do not leak database uniqueness failures.
- `AuthTokenProvider` includes the member id, active session key, and a random nonce in the token so repeated logins issue distinct token values.
- `LoginMemberArgumentResolver` remains a web adapter and delegates token/session/member validation to `AuthService`.

### Test Decisions

- `AuthTokenProviderTest` directly covers token uniqueness, payload extraction, and invalid token handling.
- `AuthServiceIntegrationTest` verifies authenticated member id and name.
- `MemberAuthenticationAcceptanceTest` covers previous-token invalidation and a real concurrent same-member login flow using latches.
