...
class ReservationAcceptanceTest extends AcceptanceTestSupport {
    ...
    @Test
    @DisplayName("이미 예약된 슬롯을 다시 예약하면 대기 예약이 생성되고 대기 순번을 조회할 수 있다")
    void duplicateReservationCreatesWaitingReservationWithRank() {
        ...
        long themeId = createTheme("어둠의 방");
        long timeId = createTime(LocalTime.of(10, 0));
        ResponseEntity<Map> confirmed = createReservation(brownToken, LocalDate.of(2030, 5, 1), timeId, themeId);
        ...
        assertThat(confirmed.getBody().get("waitingRank")).isNull();
