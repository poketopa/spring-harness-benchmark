...
public class ReservationService {
    ...
    public List<ReservationResponse> findManaged(LoginMember loginMember) {
        ...
        return reservationRepository.findAllByTheme_Store_ManagerOrderByDateAscTimeStartAtAsc(manager)
                .stream()
    ...
    private Member findManager(LoginMember loginMember) {
        Member member = authenticatedMemberService.findMember(loginMember);
        if (!member.isManager()) {
            throw new RoomescapeException(ErrorCode.UNAUTHORIZED, "매장 매니저 권한이 필요합니다.");
        }
        return member;
    }

    private Reservation findManagedReservation(Long reservationId, Member manager) {
        return reservationRepository.findByIdAndTheme_Store_Manager(reservationId, manager)
                .orElseThrow(() -> new RoomescapeException(ErrorCode.UNAUTHORIZED, "관리 중인 매장의 예약만 접근할 수 있습니다."));
    }
    ...
    @Transactional
    public void cancelManaged(LoginMember loginMember, Long reservationId) {
        Member manager = findManager(loginMember);
        Reservation reservation = findManagedReservation(reservationId, manager);
