...
public class ReservationController {
    ...
    @PostMapping({"/reservations/waiting", "/reservations/waitings", "/reservation-waitings"})
    public ResponseEntity<ReservationWaitingResponse> createWaiting(
            LoginMember loginMember,
            @Valid @RequestBody ReservationRequest request
    ) {
        ReservationWaitingResponse response = reservationService.createWaiting(loginMember, request);
        URI location = ServletUriComponentsBuilder.fromCurrentRequest()
                .path("/{id}")
                .buildAndExpand(response.id())
                .toUri();
