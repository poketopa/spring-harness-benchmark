...
public class ReservationService {
    ...
    public ReservationResponse create(LoginMember loginMember, ReservationRequest request) {
        ...
        Reservation reservation = new Reservation(member, theme, time, request.date(), ReservationStatus.CONFIRMED);
        ...
        if (reservationRepository.existsByMemberAndThemeAndTimeAndDate(member, theme, time, request.date())) {
            throw new RoomescapeException(ErrorCode.DUPLICATE_RESERVATION, "이미 예약 또는 대기 중인 시간입니다.");
        }

        boolean confirmedExists = reservationRepository.existsByThemeAndTimeAndDateAndStatus(
                theme,
                time,
                request.date(),
                ReservationStatus.CONFIRMED
        );
        ReservationStatus status = confirmedExists ? ReservationStatus.WAITING : ReservationStatus.CONFIRMED;
        reservation = new Reservation(member, theme, time, request.date(), status);
        Integer waitingRank = null;
        if (status == ReservationStatus.WAITING) {
            waitingRank = reservationRepository.countByThemeAndTimeAndDateAndStatus(
                    theme,
                    time,
                    request.date(),
        ...
        try {
            return ReservationResponse.from(reservationRepository.saveAndFlush(reservation), waitingRank);
    ...
    private Integer waitingRankOf(Reservation reservation) {
        ...
        return reservationRepository.countByThemeAndTimeAndDateAndStatusAndIdLessThanEqual(
