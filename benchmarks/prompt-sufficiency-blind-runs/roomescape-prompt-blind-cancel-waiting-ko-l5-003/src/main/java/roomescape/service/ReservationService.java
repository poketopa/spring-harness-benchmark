...
public class ReservationService {
    ...
    public void cancel(LoginMember loginMember, Long reservationId) {
        ...
        validateCancellationAllowed(member, reservation);
    ...
    private void validateCancellationAllowed(Member member, Reservation reservation) {
        if (!reservation.isOwnedBy(member)) {
            throw new RoomescapeException(ErrorCode.RESERVATION_NOT_FOUND, "예약을 찾을 수 없습니다.");
        }
        if (reservation.isPast(LocalDateTime.now(clock))) {
    ...
    private void promoteFirstWaiting(Reservation cancelledReservation) {
