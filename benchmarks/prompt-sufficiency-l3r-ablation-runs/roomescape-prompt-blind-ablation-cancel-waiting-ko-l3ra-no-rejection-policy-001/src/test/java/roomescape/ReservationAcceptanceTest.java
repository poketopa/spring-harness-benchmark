...
class ReservationAcceptanceTest extends AcceptanceTestSupport {
    ...
    void memberCancelsOwnReservation() {
        ...
        long themeId = createTheme("어둠의 방");
        long timeId = createTime(LocalTime.of(10, 0));
        ResponseEntity<Map> reservation = createReservation(token, LocalDate.of(2030, 5, 1), timeId, themeId);
        ...
        assertThat(findThemeTimes(themeId, LocalDate.of(2030, 5, 1)).getFirst().get("reserved")).isEqualTo(false);
    }

    @Test
    @DisplayName("예약이 취소되면 가장 빠른 대기자가 예약으로 승격되고 남은 대기 순번이 재계산된다")
    void cancelReservationPromotesFirstWaitingAndRecalculatesRanks() {
        ...
        ResponseEntity<Map> reservation = createReservation(brownToken, date, timeId, themeId);
