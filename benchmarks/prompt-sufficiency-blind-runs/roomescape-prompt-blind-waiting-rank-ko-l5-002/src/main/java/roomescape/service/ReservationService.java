...
public class ReservationService {
    ...
    public List<MyReservationResponse> findMine(LoginMember loginMember) {
        ...
        Stream<MyReservationResponse> reservations = reservationRepository.findAllByMemberOrderByDateAscTimeStartAtAsc(member)
                .stream()
                .map(MyReservationResponse::reserved);
        Stream<MyReservationResponse> waitings = waitingRepository.findAllByMember(member)
                .stream()
    ...
    private MyReservationResponse toWaitingResponse(Waiting waiting) {
        return MyReservationResponse.waiting(
                waiting,
                waitingRepository.rankOf(waiting.getReservation(), waiting.getCreatedAt(), waiting.getId())
        );
    }
