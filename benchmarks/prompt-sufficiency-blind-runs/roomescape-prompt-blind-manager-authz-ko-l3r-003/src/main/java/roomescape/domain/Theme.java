...
public class Theme {
    ...
    public Theme(String name, String description, String thumbnailUrl) {
        this(null, name, description, thumbnailUrl, false);
    }

    public Theme(Store store, String name, String description, String thumbnailUrl) {
        this(store, name, description, thumbnailUrl, true);
    }

    private Theme(Store store, String name, String description, String thumbnailUrl, boolean requireStore) {
        validateRequired(store, name, description, thumbnailUrl, requireStore);
        this.store = store;
        this.name = name;
    ...
    public boolean isInSameStore(ReservationTime time) {
        if (store == null || time.getStore() == null) {
            return true;
        }
        return store.isSameStore(time.getStore());
    }
}
