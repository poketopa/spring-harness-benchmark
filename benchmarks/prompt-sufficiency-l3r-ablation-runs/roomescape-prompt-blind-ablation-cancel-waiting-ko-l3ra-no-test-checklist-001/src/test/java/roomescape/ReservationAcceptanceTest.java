...
class ReservationAcceptanceTest extends AcceptanceTestSupport {
    ...
    @Test
    @DisplayName("회원이 본인 예약을 취소하면 가장 빠른 대기자가 예약으로 승격되고 남은 대기 순번이 재계산된다")
    void cancelReservationPromotesEarliestWaitingAndRecalculatesRank() {
        createMember("브라운", "brown@example.com");
        createMember("코니", "cony@example.com");
    ...
    @Test
    @DisplayName("다른 회원의 예약을 취소하면 찾을 수 없음 응답을 반환하고 예약은 유지된다")
    ...
    @Test
    @DisplayName("지난 예약을 취소하면 잘못된 요청 응답을 반환하고 예약은 유지된다")
