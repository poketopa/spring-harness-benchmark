# Roomescape JPA Auth Concurrent Login Regen V2

This is the concurrent login add-on regeneration benchmark target for the `spring-usecase-implementation`
harness project.

## Purpose

Use this project to verify whether the updated skill extends the manager authorization result with concurrent-login
prevention while preserving the owner's Spring conventions.
...
- The project uses bearer tokens, but now stores active login state on the server.
- A member can have only one active authentication session.
- A new login takes priority and invalidates the previously issued token.
- Requests with an invalidated token return `UNAUTHORIZED`.
- Requests with the latest active token continue to work.

### Policy Decision

- New login wins.
- Existing tokens are not kept in an allowlist or blacklist.
- The server stores one active token per member in `AuthSession`.
- This means the authentication flow is no longer stateless; token validation includes a database lookup.

### Design Decisions

- `AuthTokenProvider` creates a unique token per login by including a random nonce in the token payload.
- `AuthTokenProvider` only creates and parses token payloads; it does not decide session validity.
- `AuthService.login(...)` creates or renews the member's `AuthSession`.
- `AuthService.login(...)` locks the target member row while renewing the session, so simultaneous first logins for the same account do not race into duplicate session inserts.
- `AuthService.authenticate(...)` verifies both token shape and active session ownership.
- `LoginMemberArgumentResolver` still extracts the header and delegates authentication to `AuthService`.

### Test Decisions

- `AuthSessionTest` covers session token renewal and constructor invariants.
- `AuthTokenProviderTest` covers token uniqueness, member ID extraction, and invalid token handling.
- `AuthServiceTest` covers token renewal and stale-token rejection.
- `AuthSessionRepositoryIntegrationTest` covers active-session lookup and one-session-per-member persistence constraint.
- `ConcurrentLoginAcceptanceTest` covers the end-user flow: first token rejected after the second login, second token accepted.
- `ConcurrentLoginAcceptanceTest` also covers simultaneous first login requests and verifies both login responses avoid server errors while only one token remains active.
