...
class ReservationAcceptanceTest extends AcceptanceTestSupport {
    ...
    void memberCancelsOwnReservation() {
        ...
        long reservationId = ((Number) createReservation(token, date, timeId, themeId).getBody().get("id")).longValue();

        ResponseEntity<Map> response = cancelReservation(token, reservationId);

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NO_CONTENT);
        assertThat(findMine(token)).isEmpty();
        assertThat(findThemeTimes(themeId, date).getFirst().get("reserved")).isEqualTo(false);
    }

    @Test
    @DisplayName("회원은 다른 회원의 예약을 취소할 수 없다")
    void memberCannotCancelOthersReservation() {
        createMember("브라운", "brown@example.com");
        createMember("코니", "cony@example.com");
        String brownToken = login("brown@example.com");
        String conyToken = login("cony@example.com");
        ...
        long timeId = createTime(LocalTime.of(10, 0));
        LocalDate date = LocalDate.of(2030, 5, 1);
        long reservationId = ((Number) createReservation(brownToken, date, timeId, themeId).getBody().get("id")).longValue();
    ...
    void pastReservationCannotBeCanceled() {
        long memberId = createMember("브라운", "brown@example.com");
        String token = login("brown@example.com");
        long themeId = createTheme("어둠의 방");
        long timeId = createTime(LocalTime.of(10, 0));
        ...
        ReservationTime time = reservationTimeRepository.findById(timeId).get();
        Reservation reservation = reservationRepository.saveAndFlush(
                new Reservation(member, theme, time, LocalDate.of(2000, 5, 1))
        );
        ...
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
        assertThat(response.getBody().get("code")).isEqualTo("PAST_RESERVATION");
        assertThat(findMine(token)).hasSize(1);
    }
}
