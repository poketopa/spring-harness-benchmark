...
class ReservationAcceptanceTest extends AcceptanceTestSupport {
    ...
    void memberCreatesReservationAndThemeTimeBecomesReserved() {
        ...
        assertThat(created.getBody().get("status")).isEqualTo("RESERVED");
        assertThat(created.getBody().get("waitingRank")).isNull();
    ...
    @Test
    @DisplayName("이미 예약된 슬롯을 예약하면 대기 신청되고 순번을 반환한다")
    ...
    @Test
    @DisplayName("이미 대기 중인 슬롯에 다시 대기하면 충돌 응답을 반환한다")
