...
class ReservationAcceptanceTest extends AcceptanceTestSupport {
    ...
    @Test
    @DisplayName("예약을 취소하면 가장 앞선 대기가 예약으로 전환된다")
    ...
    @Test
    @DisplayName("대기가 없는 예약을 취소하면 슬롯이 비어 다시 예약할 수 있다")
    void cancelReservationWithoutWaitingMakesSlotAvailable() {
        createMember("브라운", "brown@example.com");
        createMember("코니", "cony@example.com");
        String brownToken = login("brown@example.com");
        String conyToken = login("cony@example.com");
        ...
        ResponseEntity<Void> cancelResponse = cancelReservation(brownToken, reservationId);
        ResponseEntity<Map> createResponse = createReservation(conyToken, date, timeId, themeId);

        assertThat(cancelResponse.getStatusCode()).isEqualTo(HttpStatus.NO_CONTENT);
        assertThat(createResponse.getStatusCode()).isEqualTo(HttpStatus.CREATED);
        assertThat(findMine(brownToken)).isEmpty();
        assertThat(findMine(conyToken)).hasSize(1);
    }
