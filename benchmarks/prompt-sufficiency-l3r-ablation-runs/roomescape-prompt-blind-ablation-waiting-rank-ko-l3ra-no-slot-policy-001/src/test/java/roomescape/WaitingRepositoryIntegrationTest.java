...
class WaitingRepositoryIntegrationTest {
    ...
    @Test
    @DisplayName("같은 회원의 같은 슬롯 대기를 저장하면 최종 DB 제약조건으로 실패한다")
    void finalDuplicateGuard() {
        ...
        waitingRepository.saveAndFlush(new Waiting(brown, theme, time, date, LocalDateTime.of(2030, 1, 1, 0, 0)));

        Waiting duplicate = new Waiting(brown, theme, time, date, LocalDateTime.of(2030, 1, 1, 0, 1));
