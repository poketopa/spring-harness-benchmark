...
class ReservationAcceptanceTest extends AcceptanceTestSupport {
    ...
    @Test
    @DisplayName("매니저는 관리 매장의 예약 날짜와 시간을 변경할 수 있다")
    void managerChangesOwnStoreReservationDateAndTime() {
        // given
        createMember("브라운", "brown@example.com");
        String memberToken = login("brown@example.com");
        String managerToken = createManagerForDefaultStore("매니저", "manager@example.com");
        long themeId = createTheme("어둠의 방");
        ...
        List<Map<String, Object>> originalTimes = findThemeTimes(themeId, originalDate);
        assertThat(originalTimes)
                .filteredOn(time -> ((Number) time.get("id")).longValue() == tenOClockId)
                .first()
                .extracting(time -> time.get("reserved"))
                .isEqualTo(false);

        List<Map<String, Object>> changedTimes = findThemeTimes(themeId, changedDate);
        assertThat(changedTimes)
                .filteredOn(time -> ((Number) time.get("id")).longValue() == elevenOClockId)
                .first()
                .extracting(time -> time.get("reserved"))
                .isEqualTo(true);
    }
    ...
    @Test
    @DisplayName("매니저는 관리 매장의 예약을 취소할 수 있다")
    void managerCancelsOwnStoreReservation() {
        ...
        long themeId = createTheme("어둠의 방");
        long timeId = createTime(LocalTime.of(10, 0));
        long reservationId = ((Number) createReservation(memberToken, LocalDate.of(2030, 5, 1), timeId, themeId)
                .getBody()
                .get("id")).longValue();
    ...
    @Test
    @DisplayName("예약 취소 시 첫 대기가 예약으로 전환되고 다음 대기의 순번은 앞당겨진다")
    ...
    @Test
    @DisplayName("이미 예약된 슬롯으로 예약을 변경하면 충돌 응답을 반환한다")
    void changeReservationToOccupiedSlotReturnsConflict() {
        // given
        createMember("브라운", "brown@example.com");
        createMember("코니", "cony@example.com");
        String brownToken = login("brown@example.com");
        ...
        long elevenOClockId = createTime(LocalTime.of(11, 0));
        LocalDate date = LocalDate.of(2030, 5, 1);
        long brownReservationId = ((Number) createReservation(brownToken, date, tenOClockId, themeId)
                .getBody()
                .get("id")).longValue();
        createReservation(conyToken, date, elevenOClockId, themeId);

        // when
        ResponseEntity<Map> response = changeReservation(managerToken, brownReservationId, date, elevenOClockId);

        // then
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.CONFLICT);
        assertThat(response.getBody().get("code")).isEqualTo("DUPLICATE_RESERVATION");
    }
