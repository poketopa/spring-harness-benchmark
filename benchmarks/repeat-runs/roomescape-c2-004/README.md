...
- A member can change the date and time of their own future reservation.
- A member cannot change a reservation to a past slot or an already reserved slot.
- A member can cancel their own future reservation.
- Missing or non-owned reservations are returned as `RESERVATION_NOT_FOUND`.
- Cancelling a reservation automatically promotes the first waiting entry for the same slot to a reservation.
- Waiting ranks are recalculated from the remaining waiting order after cancellation or promotion.
- Authenticated member lookup is centralized before reservation or waiting services apply use-case rules.
...
- `PUT /reservations/{reservationId}`
  ...
  - Fails with `RESERVATION_NOT_FOUND`, `PAST_RESERVATION`, or `DUPLICATE_RESERVATION`.
...
- Reservation cancellation and waiting promotion are one transaction because the original reservation deletion, first waiting deletion, and promoted reservation creation must remain consistent.
- Automatic promotion was chosen because it keeps the user flow simple: when a confirmed reservation frees a slot, the first waiting member receives it immediately.
