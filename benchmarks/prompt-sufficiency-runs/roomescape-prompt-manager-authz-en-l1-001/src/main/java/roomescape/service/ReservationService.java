...
public class ReservationService {
    ...
    public List<ReservationResponse> findManagerReservations(LoginMember loginMember) {
        Member manager = managerAuthorizationService.authorizeManager(loginMember);
    ...
    @Transactional
    public ReservationResponse changeByManager(
            LoginMember loginMember,
            Long reservationId,
            ReservationChangeRequest request
    ) {
        Reservation reservation = findManagerReservation(loginMember, reservationId);
        ReservationTime time = reservationTimeRepository.getByIdOrThrow(request.timeId());
    ...
    @Transactional
    public void cancelByManager(LoginMember loginMember, Long reservationId) {
        Reservation reservation = findManagerReservation(loginMember, reservationId);
    ...
    private Reservation findManagerReservation(LoginMember loginMember, Long reservationId) {
        Member manager = managerAuthorizationService.authorizeManager(loginMember);
        Reservation reservation = reservationRepository.findWithThemeStoreById(reservationId)
                .orElseThrow(() -> new RoomescapeException(ErrorCode.RESERVATION_NOT_FOUND, "예약을 찾을 수 없습니다."));
        managerAuthorizationService.authorizeStoreManager(manager, reservation);
        return reservation;
    }
