...
abstract class AcceptanceTestSupport {
    ...
    protected String adminToken() {
        if (adminToken == null) {
            createAdminFixture("어드민", "admin@example.com");
            adminToken = login("admin@example.com");
        }
        return adminToken;
    }

    private void createAdminFixture(String name, String email) {
        if (memberRepository.findByEmail(email).isEmpty()) {
            memberRepository.save(new Member(name, email, "password", Role.ADMIN));
        }
    }
    ...
    protected ResponseEntity<Map> changeManagedReservation(String token, long reservationId, LocalDate date, long timeId) {
        Map<String, Object> request = Map.of(
                "date", date.toString(),
                "timeId", timeId
        );
