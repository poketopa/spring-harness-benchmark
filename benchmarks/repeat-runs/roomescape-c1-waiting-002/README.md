...
- `POST /waitings`: a logged-in member can wait for an already reserved theme/date/time slot.
- `DELETE /waitings/{waitingId}`: a logged-in member can cancel only their own waiting.
- `GET /reservations/mine`: returns both reservations and waitings with `status`; waitings include `rank`.

Implementation decisions:

- `Waiting` is a peer domain/entity, not a reservation subtype.
- Waiting use cases live in `WaitingService`.
- Combined personal reservation history lives in `MyReservationService`.
- Waiting rank is calculated by a direct repository count query for earlier waitings in the same slot.
- Duplicate waitings are guarded by both service validation and a database unique constraint.
