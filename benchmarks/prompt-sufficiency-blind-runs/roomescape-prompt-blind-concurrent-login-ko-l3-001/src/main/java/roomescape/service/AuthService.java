...
import roomescape.auth.AccessTokenPayload;
...
public class AuthService {
    ...
    public LoginResponse login(LoginRequest request) {
        Member member = memberRepository.findLockedByEmail(request.email())
        ...
        member.renewLoginToken(loginTokenId);
        return new LoginResponse(authTokenProvider.createToken(member, loginTokenId));
    }

    public LoginMember authenticate(String accessToken) {
        AccessTokenPayload payload = authTokenProvider.extractPayload(accessToken);
        ...
        if (!member.hasLoginToken(payload.loginTokenId())) {
