...
public class ReservationService {
    ...
    public ReservationResponse change(LoginMember loginMember, Long reservationId, ReservationChangeRequest request) {
        Member member = authenticatedMemberService.findMember(loginMember);
        Reservation reservation = findChangeableReservation(reservationId, member);
        ReservationTime time = reservationTimeRepository.getByIdOrThrow(request.timeId());

        return changeSchedule(reservation, time, request);
    }

    private Reservation findChangeableReservation(Long reservationId, Member member) {
        if (member.getRole() == Role.MANAGER) {
            return findManagedReservation(reservationId, member);
        }
        return findOwnReservation(reservationId, member);
    }
    ...
    private Reservation findManagedReservation(Long reservationId, Member manager) {
        ...
        validateManagedReservation(reservation, manager);
        return reservation;
    }
    ...
    private void validateManagedReservation(Reservation reservation, Member manager) {
