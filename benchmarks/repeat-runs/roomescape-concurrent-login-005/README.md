...
This target uses a "new login wins" policy. When the same member logs in again, the newly issued token becomes the
only active token and the previous token is rejected as `UNAUTHORIZED`.

`roomescape-repeat-concurrent-login-004` was recorded as a failed run because stale-token status assertions attempted
to deserialize an unauthorized error body as a success list. This target avoids that by using `String` responses when
only the unauthorized status is asserted.

### Design Decisions

- `AuthSession` stores one active token id per member.
- Login locks the target member row with a pessimistic write lock before renewing the session.
- `AuthTokenProvider` issues nonce-bearing tokens so repeated logins for the same member produce unique tokens.
- Authentication validates both the token payload and the server-side active session state.
- `LoginMemberArgumentResolver` remains a web adapter and delegates authentication policy to `AuthService`.

### Test Decisions

- `AuthTokenProviderTest` covers token uniqueness, payload extraction, and invalid token conversion.
- `MemberAuthenticationAcceptanceTest` covers stale-token rejection, login identity, and a real concurrent login flow
  using latches.
