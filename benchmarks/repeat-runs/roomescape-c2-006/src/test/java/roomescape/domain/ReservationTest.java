...
class ReservationTest {
    ...
    void changeReservationSchedule() {
        Member member = new Member("브라운", "brown@example.com", "password");
        Theme theme = new Theme("어둠의 방", "방탈출", "https://example.com/dark.jpg");
        ReservationTime firstTime = new ReservationTime(LocalTime.of(10, 0));
        ReservationTime secondTime = new ReservationTime(LocalTime.of(11, 0));
        Reservation reservation = new Reservation(member, theme, firstTime, LocalDate.of(2030, 5, 1));

        reservation.changeSchedule(secondTime, LocalDate.of(2030, 5, 2));

        assertThat(reservation.getTime()).isEqualTo(secondTime);
