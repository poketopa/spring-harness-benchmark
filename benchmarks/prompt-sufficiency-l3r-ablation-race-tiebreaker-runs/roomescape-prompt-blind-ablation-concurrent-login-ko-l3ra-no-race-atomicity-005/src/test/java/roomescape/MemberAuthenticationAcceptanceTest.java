...
class MemberAuthenticationAcceptanceTest extends AcceptanceTestSupport {
    ...
    @Test
    @DisplayName("같은 회원이 다시 로그인하면 새 토큰만 유효하고 예전 토큰은 인증 실패한다")
    void latestLoginTokenOnlyRemainsValid() {
    ...
    void otherMemberTokenRemainsValidAfterLoginRenewal() {
        createMember("브라운", "brown@example.com");
    ...
    @Test
    @DisplayName("같은 회원이 동시에 로그인해도 하나의 최신 토큰만 유효하다")
    void concurrentLoginLeavesOnlyOneValidToken() throws Exception {
        ...
        try {
            ...
            assertThat(List.of(
                    findMineResponse(firstToken).getStatusCode(),
                    findMineResponse(secondToken).getStatusCode()
