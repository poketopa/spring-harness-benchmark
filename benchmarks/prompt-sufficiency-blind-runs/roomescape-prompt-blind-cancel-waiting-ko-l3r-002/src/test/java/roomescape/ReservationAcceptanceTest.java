...
class ReservationAcceptanceTest extends AcceptanceTestSupport {
    ...
    void cancelReservationPromotesFirstWaitingAndRecalculatesRanks() {
        ...
        createWaiting(dukeToken, date, timeId, themeId);
        ...
        List<Map<String, Object>> dukeMine = findMine(dukeToken);
        ...
        assertThat(dukeMine.getFirst().get("status")).isEqualTo("WAITING");
        assertThat(dukeMine.getFirst().get("rank")).isEqualTo(2);
        assertThat(findThemeTimes(themeId, date).getFirst().get("reserved")).isEqualTo(true);
    }
}
