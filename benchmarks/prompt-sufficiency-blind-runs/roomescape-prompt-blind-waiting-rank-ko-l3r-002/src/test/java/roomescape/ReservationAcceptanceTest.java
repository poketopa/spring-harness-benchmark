...
class ReservationAcceptanceTest extends AcceptanceTestSupport {
    ...
    @Test
    @DisplayName("이미 예약된 슬롯에는 대기를 신청하고 순번을 조회할 수 있다")
    void occupiedSlotWaitingReturnsRank() {
        ...
        assertThat(response.getBody().get("memberId")).isEqualTo((int) conyId);
    ...
    void availableSlotWaitingReturnsBadRequest() {
        ...
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
        assertThat(response.getBody().get("code")).isEqualTo("WAITING_SLOT_AVAILABLE");
    }
    ...
    void ownReservationWaitingReturnsBadRequest() {
        ...
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
        assertThat(response.getBody().get("code")).isEqualTo("WAITING_OWN_RESERVATION");
    }
    ...
    void waitingRankOrdersByCreateOrder() {
        createMember("브라운", "brown@example.com");
        createMember("코니", "cony@example.com");
        createMember("샐리", "sally@example.com");
        createMember("초코", "choco@example.com");
        String brownToken = login("brown@example.com");
        String conyToken = login("cony@example.com");
        String sallyToken = login("sally@example.com");
        String chocoToken = login("choco@example.com");
        long themeId = createTheme("어둠의 방");
        long timeId = createTime(LocalTime.of(10, 0));
        createReservation(brownToken, LocalDate.of(2030, 5, 1), timeId, themeId);
        ...
        ResponseEntity<Map> third = createWaiting(chocoToken, LocalDate.of(2030, 5, 1), timeId, themeId);
    ...
    @Test
    @DisplayName("내 예약 목록은 예약과 대기를 함께 보여주고 대기에는 현재 순번을 표시한다")
    void myListContainsReservationsAndWaitingsWithRank() {
        ...
        createReservation(brownToken, LocalDate.of(2030, 5, 1), waitingTimeId, themeId);
        createWaiting(conyToken, LocalDate.of(2030, 5, 1), waitingTimeId, themeId);
        createReservation(conyToken, LocalDate.of(2030, 5, 2), reservationTimeId, themeId);
        ...
        assertThat(mine).anySatisfy(item -> {
            ...
            assertThat(item.get("rank")).isNull();
        });
    }
}
