...
public class AuthTokenProvider {
    ...
    public String createToken(Member member, String loginKey) {
        String payload = PREFIX + member.getId() + DELIMITER + loginKey;
        return Base64.getUrlEncoder()
                .withoutPadding()
                .encodeToString(payload.getBytes(StandardCharsets.UTF_8));
    }

    public LoginToken extractLoginToken(String token) {
        try {
            ...
            return new LoginToken(Long.parseLong(values[0]), values[1]);
        } catch (RuntimeException exception) {
            throw new RoomescapeException(ErrorCode.UNAUTHORIZED, "유효하지 않은 인증 정보입니다.");
        }
    }

    public record LoginToken(Long memberId, String loginKey) {
    }
}
