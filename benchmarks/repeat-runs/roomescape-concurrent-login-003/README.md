...
- New login wins.
- A successful login rotates the member's active server-side auth session.
- Previously issued tokens fail authentication after a later login.

### Implementation Decisions

- `AuthSession` stores one active session key per member.
- `AuthService.login(...)` locks the member row, renews the active session, flushes it, and then issues a token.
- `AuthService.authenticate(...)` validates both token payload and active server-side session state.
- `LoginMemberArgumentResolver` remains a web adapter and delegates authentication to `AuthService`.
- Tokens contain member id, session key, and a random nonce so repeated logins produce unique tokens.

### Test Decisions

- `AuthTokenProviderTest` directly covers token uniqueness, parsing, and invalid-token behavior.
- `AuthServiceIntegrationTest` covers stale token rejection and `LoginMember` identity.
- `ConcurrentLoginAcceptanceTest` uses latches to verify that concurrent login leaves exactly one active token.
