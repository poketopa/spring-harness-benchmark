...
public class AuthService {
    ...
    public LoginResponse login(LoginRequest request) {
        ...
        Member lockedMember = memberRepository.findByIdForUpdate(member.getId())
                .orElseThrow(() -> new RoomescapeException(ErrorCode.UNAUTHORIZED, "이메일 또는 비밀번호가 올바르지 않습니다."));
        String sessionKey = renewSession(lockedMember);
        return new LoginResponse(authTokenProvider.createToken(lockedMember, sessionKey));
    }

    private String renewSession(Member member) {
        String sessionKey = UUID.randomUUID().toString();
        AuthSession authSession = authSessionRepository.findByMember(member)
                .orElseGet(() -> new AuthSession(member, sessionKey));
        authSession.rotate(sessionKey);
    ...
    public LoginMember authenticate(String accessToken) {
        ...
        String sessionKey = authTokenProvider.extractSessionKey(accessToken);
        Member member = memberRepository.findById(memberId)
                .orElseThrow(() -> new RoomescapeException(ErrorCode.UNAUTHORIZED, "유효하지 않은 인증 정보입니다."));
        if (!authSessionRepository.existsByMemberAndSessionKey(member, sessionKey)) {
            throw new RoomescapeException(ErrorCode.UNAUTHORIZED, "만료된 인증 정보입니다.");
        }
