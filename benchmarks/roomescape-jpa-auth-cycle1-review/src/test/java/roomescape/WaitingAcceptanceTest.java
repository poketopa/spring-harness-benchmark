...
class WaitingAcceptanceTest extends AcceptanceTestSupport {
    ...
    @Test
    @DisplayName("회원은 이미 예약된 슬롯에 대기를 신청하고 내 목록에서 순번을 확인할 수 있다")
    void membersCreateWaitingForReservedSlotAndFindMineWithReservationStatus() {
        ...
        assertThat(conyWaiting.getBody().get("waitingRank")).isEqualTo(1);
        assertThat(sallyWaiting.getBody().get("waitingRank")).isEqualTo(2);

        List<Map<String, Object>> brownMine = findMine(brownToken);
        assertThat(brownMine).hasSize(1);
        assertThat(brownMine.getFirst().get("status")).isEqualTo("RESERVATION");
        ...
        assertThat(conyMine.getFirst().get("waitingRank")).isEqualTo(1);
    }
    ...
    @Test
    @DisplayName("예약되지 않은 슬롯이나 본인이 예약한 슬롯에 대기하면 잘못된 요청 응답을 반환한다")
    void waitingForUnreservedOrOwnReservedSlotReturnsBadRequest() {
        createMember("브라운", "brown@example.com");
        createMember("코니", "cony@example.com");
        String brownToken = login("brown@example.com");
        String conyToken = login("cony@example.com");
        ...
        ResponseEntity<Map> unreserved = createWaiting(conyToken, date, timeId, themeId);

        assertThat(unreserved.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
        assertThat(unreserved.getBody().get("code")).isEqualTo("UNAVAILABLE_WAITING");

        createReservation(brownToken, date, timeId, themeId);

        ResponseEntity<Map> ownReserved = createWaiting(brownToken, date, timeId, themeId);

        assertThat(ownReserved.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
        assertThat(ownReserved.getBody().get("code")).isEqualTo("UNAVAILABLE_WAITING");
    }
    ...
    void memberCancelsOwnWaiting() {
        ...
        ResponseEntity<Void> response = cancelWaiting(conyToken, ((Number) waiting.getBody().get("id")).longValue());
