...
public class ReservationService {
    ...
    public List<ReservationResponse> findMine(LoginMember loginMember) {
        ...
        List<ReservationResponse> reservations = reservationRepository.findAllByMemberOrderByDateAscTimeStartAtAsc(member)
                ...
                .toList();
        List<ReservationResponse> waitings = waitingRepository.findAllByMemberOrderByDateAscTimeStartAtAscAppliedAtAscIdAsc(member)
                .stream()
                .map(waiting -> ReservationResponse.from(
                        waiting,
                        waitingRepository.countRank(
                                waiting.getTheme(),
                                waiting.getTime(),
                                waiting.getDate(),
                                waiting.getAppliedAt(),
                                waiting.getId()
                        )
                ))
                .toList();
