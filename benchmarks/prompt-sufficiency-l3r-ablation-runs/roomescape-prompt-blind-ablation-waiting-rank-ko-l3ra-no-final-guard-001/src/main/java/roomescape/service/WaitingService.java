...
public class WaitingService {
    ...
    public ReservationResponse create(LoginMember loginMember, ReservationRequest request) {
        ...
        validateCreateAllowed(member, theme, time, waiting, now);

        try {
            Waiting saved = waitingRepository.saveAndFlush(waiting);
            int rank = waitingRepository.countRank(theme, time, request.date(), saved.getCreatedAt(), saved.getId());
            return ReservationResponse.fromWaiting(saved, rank);
        } catch (DataIntegrityViolationException exception) {
            throw new RoomescapeException(ErrorCode.DUPLICATE_WAITING, "이미 대기 신청한 예약입니다.");
        }
    }

    private void validateCreateAllowed(Member member, Theme theme, ReservationTime time, Waiting waiting, LocalDateTime now) {
        if (waiting.isPast(now)) {
            throw new RoomescapeException(ErrorCode.PAST_RESERVATION, "지난 날짜와 시간은 대기할 수 없습니다.");
        }

        Reservation reservation = reservationRepository.findByThemeAndTimeAndDate(theme, time, waiting.getDate())
                .orElseThrow(() -> new RoomescapeException(ErrorCode.AVAILABLE_SLOT_WAITING, "예약 가능한 시간에는 대기할 수 없습니다."));

        if (reservation.isOwnedBy(member)) {
            throw new RoomescapeException(ErrorCode.OWN_RESERVATION_WAITING, "본인의 예약에는 대기할 수 없습니다.");
        }
        if (waitingRepository.existsByMemberAndThemeAndTimeAndDate(member, theme, time, waiting.getDate())) {
            throw new RoomescapeException(ErrorCode.DUPLICATE_WAITING, "이미 대기 신청한 예약입니다.");
        }
    }
