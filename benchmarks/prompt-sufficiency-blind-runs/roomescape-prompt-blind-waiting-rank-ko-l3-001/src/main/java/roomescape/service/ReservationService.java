...
public class ReservationService {
    ...
    public ReservationResponse create(LoginMember loginMember, ReservationRequest request) {
        ...
        return reservationRepository.findByThemeAndTimeAndDate(theme, time, request.date())
                .map(existingReservation -> createWaiting(member, theme, time, request, existingReservation))
    ...
    public List<ReservationResponse> findMine(LoginMember loginMember) {
        ...
        List<ReservationResponse> waitings = waitingRepository.findAllByMemberOrderByDateAscTimeStartAtAsc(member)
                .stream()
                .map(waiting -> ReservationResponse.of(waiting, waitingRank(waiting)))
                .toList();

        return java.util.stream.Stream.concat(reservations.stream(), waitings.stream())
                .sorted(java.util.Comparator.comparing(ReservationResponse::date)
    ...
    private ReservationResponse createWaiting(
        ...
        if (existingReservation.isOwnedBy(member)) {
        ...
        try {
            Waiting waiting = waitingRepository.saveAndFlush(new Waiting(member, theme, time, request.date()));
            return ReservationResponse.of(waiting, waitingRank(waiting));
    ...
    private int waitingRank(Waiting waiting) {
        return Math.toIntExact(waitingRepository.countByThemeAndTimeAndDateAndIdLessThanEqual(
                waiting.getTheme(),
                waiting.getTime(),
                waiting.getDate(),
                waiting.getId()
