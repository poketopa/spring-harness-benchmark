...
abstract class AcceptanceTestSupport {
    ...
    @Autowired
    protected StoreManagerRepository storeManagerRepository;
    ...
    protected String createManagerForDefaultStore(String name, String email) {
        Member manager = memberRepository.save(new Member(name, email, "password", Role.MANAGER));
        Store store = storeRepository.getDefaultStore();
    ...
    protected ResponseEntity<Map> findReservation(String token, long reservationId) {
        return restTemplate.exchange(
                "/reservations/" + reservationId,
                HttpMethod.GET,
    ...
    protected ResponseEntity<List> findStoreReservations(String token, long storeId) {
        return restTemplate.exchange(
