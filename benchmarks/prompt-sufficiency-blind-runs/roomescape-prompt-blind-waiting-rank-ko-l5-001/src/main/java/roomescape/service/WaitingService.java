...
public class WaitingService {
    ...
    public WaitingResponse create(LoginMember loginMember, WaitingRequest request) {
        ...
        try {
            ...
            return WaitingResponse.of(saved, rankOf(saved));
        } catch (DataIntegrityViolationException exception) {
            throw new RoomescapeException(ErrorCode.DUPLICATE_WAITING, "이미 대기 신청한 예약입니다.");
        }
    }

    public List<MyReservationResponse> findMine(Member member) {
        return waitingRepository.findAllByMemberOrderByDateAscTimeStartAtAscRequestedAtAscIdAsc(member)
                .stream()
    ...
    private Reservation findOccupiedReservation(Theme theme, ReservationTime time, LocalDate date) {
        return reservationRepository.findByThemeAndTimeAndDate(theme, time, date)
                .orElseThrow(() -> new RoomescapeException(ErrorCode.AVAILABLE_SLOT_WAITING, "예약된 슬롯에만 대기할 수 있습니다."));
    }

    private int rankOf(Waiting waiting) {
