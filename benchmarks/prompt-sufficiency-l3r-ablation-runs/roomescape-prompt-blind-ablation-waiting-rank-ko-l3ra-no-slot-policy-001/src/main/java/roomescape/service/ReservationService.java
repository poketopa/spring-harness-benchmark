...
public class ReservationService {
    ...
    public ReservationResponse create(LoginMember loginMember, ReservationRequest request) {
        ...
        if (reservationRepository.existsByThemeAndTimeAndDate(theme, time, request.date())) {
            return createWaiting(member, theme, time, request.date());
        }
    ...
    private ReservationResponse createWaiting(Member member, Theme theme, ReservationTime time, LocalDate date) {
        ...
        Waiting waiting = new Waiting(member, theme, time, date, LocalDateTime.now(clock));
        try {
            Waiting saved = waitingRepository.saveAndFlush(waiting);
            int rank = waitingRepository.rankOf(theme, time, date, saved.getRequestedAt(), saved.getId());
            return ReservationResponse.ofWaiting(saved, rank);
        } catch (DataIntegrityViolationException exception) {
            throw new RoomescapeException(ErrorCode.DUPLICATE_WAITING, "이미 대기 신청한 시간입니다.");
        }
    }
    ...
    public List<ReservationResponse> findMine(LoginMember loginMember) {
        ...
        List<ReservationResponse> reservations = reservationRepository.findAllByMemberOrderByDateAscTimeStartAtAsc(member)
                ...
                .toList();
        List<ReservationResponse> waitings = waitingRepository.findAllByMemberOrderByDateAscTimeStartAtAscRequestedAtAscIdAsc(member)
                .stream()
                .map(waiting -> ReservationResponse.ofWaiting(waiting, waitingRepository.rankOf(
                        waiting.getTheme(),
                        waiting.getTime(),
                        waiting.getDate(),
        ...
        return Stream.concat(reservations.stream(), waitings.stream())
                .sorted(Comparator
                        ...
                        .thenComparing(response -> "RESERVED".equals(response.status()) ? 0 : 1)
