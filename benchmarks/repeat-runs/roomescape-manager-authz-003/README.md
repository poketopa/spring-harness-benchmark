# Roomescape JPA Auth Cycle2 Regen V2

This is the second regeneration benchmark target for the `spring-usecase-implementation` harness project.

## Purpose

Use this project to verify whether the updated skill regenerates cycle2 reservation/waiting behavior with the owner's Spring conventions.
...
- Admin can create a store for a member with `MANAGER` role.
- Admin can create a theme assigned to a store.
- A manager can list reservations for their own store through `GET /manager/reservations`.
- A manager can change or cancel only reservations for their own store through `/manager/reservations/{reservationId}`.
- Missing credentials return `UNAUTHORIZED`; logged-in users without manager/store authority return `FORBIDDEN`.
...
- `POST /admin/stores`
  ...
  - A manager can own one store in this benchmark.
- `POST /admin/themes`
  - Existing request remains compatible.
  - Optional `storeId` assigns the theme to a store.
- `GET /manager/reservations`
  - Authenticated manager only.
  - Response: own-store reservations.
- `PUT /manager/reservations/{reservationId}` and `DELETE /manager/reservations/{reservationId}`
  - Authenticated manager only.
  - Cross-store access returns `FORBIDDEN`.

### Authorization Decisions

- `ManagerAuthorizationService` owns manager role and managed-store authorization.
- `Store` owns the invariant that its manager must have `Role.MANAGER`.
- `/manager/reservations` remains in `ReservationController` because reservation endpoints are grouped by resource in this project.
