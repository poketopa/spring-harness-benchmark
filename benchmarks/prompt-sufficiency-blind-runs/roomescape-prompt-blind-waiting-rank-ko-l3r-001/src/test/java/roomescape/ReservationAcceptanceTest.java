...
class ReservationAcceptanceTest extends AcceptanceTestSupport {
    ...
    @Test
    @DisplayName("예약이 차 있는 슬롯에는 대기를 신청하고 현재 순번을 확인할 수 있다")
    void memberCreatesWaitingForOccupiedSlot() {
        createMember("브라운", "brown@example.com");
        createMember("코니", "cony@example.com");
        String brownToken = login("brown@example.com");
        String conyToken = login("cony@example.com");
    ...
    void availableSlotWaitingReturnsBadRequest() {
        ...
        assertThat(waiting.getBody().get("code")).isEqualTo("WAITING_UNAVAILABLE");
    }
    ...
    void waitingRankFollowsCreationOrder() {
        createMember("브라운", "brown@example.com");
        createMember("코니", "cony@example.com");
    ...
    @Test
    @DisplayName("내 예약 목록에서는 예약과 대기를 함께 보여 주고 대기 항목의 현재 순번을 표시한다")
    void myListContainsReservationsAndWaitingsWithRank() {
        createMember("브라운", "brown@example.com");
        createMember("코니", "cony@example.com");
        String brownToken = login("brown@example.com");
        String conyToken = login("cony@example.com");
        long darkThemeId = createTheme("어둠의 방");
        long oceanThemeId = createTheme("바다의 방");
        long timeId = createTime(LocalTime.of(10, 0));
        LocalDate date = LocalDate.of(2030, 5, 1);
        createReservation(brownToken, date, timeId, darkThemeId);
        createReservation(conyToken, date, timeId, oceanThemeId);
        createWaiting(conyToken, date, timeId, darkThemeId);
        ...
        assertThat(mine)
                ...
                .containsExactlyInAnyOrder("RESERVATION", "WAITING");
        Map<String, Object> waiting = mine.stream()
                .filter(item -> "WAITING".equals(item.get("status")))
                .findFirst()
                .orElseThrow();
        assertThat(waiting.get("themeName")).isEqualTo("어둠의 방");
