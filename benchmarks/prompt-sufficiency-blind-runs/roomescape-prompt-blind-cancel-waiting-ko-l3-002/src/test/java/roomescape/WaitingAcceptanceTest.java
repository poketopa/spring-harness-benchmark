...
class WaitingAcceptanceTest extends AcceptanceTestSupport {
    ...
    @Test
    @DisplayName("예약이 취소되면 첫 번째 대기자가 예약되고 다음 대기자의 순번이 앞으로 당겨진다")
    void cancelReservationPromotesFirstWaiting() {
        ...
        List<Map<String, Object>> times = findThemeTimes(themeId, date);
        assertThat(times.getFirst().get("reserved")).isEqualTo(true);
    }
}
