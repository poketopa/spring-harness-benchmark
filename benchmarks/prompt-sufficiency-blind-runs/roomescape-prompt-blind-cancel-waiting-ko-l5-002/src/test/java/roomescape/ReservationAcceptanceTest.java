...
class ReservationAcceptanceTest extends AcceptanceTestSupport {
    ...
    void memberCancelsOwnReservation() {
        ...
        long timeId = createTime(LocalTime.of(10, 0));
        ResponseEntity<Map> created = createReservation(token, LocalDate.of(2030, 5, 1), timeId, themeId);
        long reservationId = ((Number) created.getBody().get("id")).longValue();

        ResponseEntity<Map> response = cancelReservation(token, reservationId);
    ...
    void pastReservationCancelReturnsBadRequest() {
        createMember("브라운", "brown@example.com");
        String token = login("brown@example.com");
        long themeId = createTheme("어둠의 방");
        long timeId = createTime(LocalTime.of(10, 0));
        Reservation reservation = reservationRepository.save(new Reservation(
                memberRepository.findByEmail("brown@example.com").orElseThrow(),
                themeRepository.findById(themeId).orElseThrow(),
                timeRepository.findById(timeId).orElseThrow(),
                LocalDate.of(2000, 5, 1)
        ));

        ResponseEntity<Map> response = cancelReservation(token, reservation.getId());
    ...
    void otherMemberReservationCancelReturnsNotFound() {
        createMember("브라운", "brown@example.com");
        createMember("코니", "cony@example.com");
        String brownToken = login("brown@example.com");
        String conyToken = login("cony@example.com");
        long themeId = createTheme("어둠의 방");
        long timeId = createTime(LocalTime.of(10, 0));
        ResponseEntity<Map> created = createReservation(brownToken, LocalDate.of(2030, 5, 1), timeId, themeId);
        long reservationId = ((Number) created.getBody().get("id")).longValue();

        ResponseEntity<Map> response = cancelReservation(conyToken, reservationId);
    ...
    @Test
    @DisplayName("예약을 취소하면 같은 슬롯의 가장 빠른 대기가 예약으로 승격되고 남은 대기 순번이 재계산된다")
    void cancelReservationPromotesFirstWaitingAndRecalculatesRanks() {
        ...
        ResponseEntity<Map> created = createReservation(brownToken, date, timeId, themeId);
        long reservationId = ((Number) created.getBody().get("id")).longValue();
        createWaiting(conyToken, date, timeId, themeId);
        createWaiting(sallyToken, date, timeId, themeId);

        ResponseEntity<Map> response = cancelReservation(brownToken, reservationId);
