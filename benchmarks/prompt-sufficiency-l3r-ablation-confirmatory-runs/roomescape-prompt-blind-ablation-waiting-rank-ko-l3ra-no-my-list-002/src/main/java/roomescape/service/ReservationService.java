...
public class ReservationService {
    ...
    @Transactional
    public ReservationWaitingResponse createWaiting(LoginMember loginMember, ReservationWaitingRequest request) {
        Member member = findMember(loginMember);
        Theme theme = findTheme(request.themeId());
        ReservationTime time = findTime(request.timeId());
        Reservation reservation = reservationRepository.findByThemeAndTimeAndDate(theme, time, request.date())
                .orElseThrow(() -> new RoomescapeException(
                        ErrorCode.AVAILABLE_RESERVATION_WAITING,
                        "예약 가능한 시간에는 대기를 신청할 수 없습니다."
                ));

        if (reservation.isOwnedBy(member)) {
            throw new RoomescapeException(ErrorCode.OWN_RESERVATION_WAITING, "본인의 예약에는 대기를 신청할 수 없습니다.");
        }
        if (reservationWaitingRepository.existsByMemberAndThemeAndTimeAndDate(member, theme, time, request.date())) {
            throw new RoomescapeException(ErrorCode.DUPLICATE_RESERVATION_WAITING, "이미 대기를 신청한 시간입니다.");
        }
        ...
        try {
            ...
            long rank = reservationWaitingRepository.countRankBySlot(
                    theme,
                    time,
                    request.date(),
                    saved.getCreatedAt(),
            ...
            return ReservationWaitingResponse.of(saved, rank);
        } catch (DataIntegrityViolationException exception) {
            throw new RoomescapeException(ErrorCode.DUPLICATE_RESERVATION_WAITING, "이미 대기를 신청한 시간입니다.");
        }
    }
