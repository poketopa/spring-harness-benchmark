...
class ReservationAcceptanceTest extends AcceptanceTestSupport {
    ...
    @Test
    @DisplayName("이미 예약된 슬롯에는 대기를 신청할 수 있고 대기 순번을 반환한다")
    void occupiedSlotWaitingReturnsRank() {
        ...
        ResponseEntity<Map> waiting = createReservationWaiting(conyToken, LocalDate.of(2030, 5, 1), timeId, themeId);

        assertThat(waiting.getStatusCode()).isEqualTo(HttpStatus.CREATED);
        assertThat(waiting.getHeaders().getLocation()).isNotNull();
        assertThat(waiting.getBody().get("memberName")).isEqualTo("코니");
    ...
    void availableSlotWaitingReturnsBadRequest() {
        ...
        assertThat(waiting.getBody().get("code")).isEqualTo("AVAILABLE_RESERVATION_WAITING");
    }
    ...
    void ownReservationWaitingReturnsBadRequest() {
        ...
        ResponseEntity<Map> waiting = createReservationWaiting(brownToken, LocalDate.of(2030, 5, 1), timeId, themeId);
    ...
    void duplicateWaitingReturnsConflict() {
        ...
        long timeId = createTime(LocalTime.of(10, 0));
        createReservation(brownToken, LocalDate.of(2030, 5, 1), timeId, themeId);
        createReservationWaiting(conyToken, LocalDate.of(2030, 5, 1), timeId, themeId);

        ResponseEntity<Map> duplicate = createReservationWaiting(conyToken, LocalDate.of(2030, 5, 1), timeId, themeId);

        assertThat(duplicate.getStatusCode()).isEqualTo(HttpStatus.CONFLICT);
        assertThat(duplicate.getBody().get("code")).isEqualTo("DUPLICATE_RESERVATION_WAITING");
    }

    @Test
    @DisplayName("대기 순번은 신청 순서와 식별자 순서로 결정된다")
    void waitingRankOrderingFollowsCreationOrderAndId() {
        createMember("브라운", "brown@example.com");
        createMember("코니", "cony@example.com");
        ...
        ResponseEntity<Map> first = createReservationWaiting(conyToken, LocalDate.of(2030, 5, 1), timeId, themeId);
        ResponseEntity<Map> second = createReservationWaiting(sallyToken, LocalDate.of(2030, 5, 1), timeId, themeId);
