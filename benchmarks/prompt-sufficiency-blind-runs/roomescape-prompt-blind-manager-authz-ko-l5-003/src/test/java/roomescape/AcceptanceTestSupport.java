...
abstract class AcceptanceTestSupport {
    ...
    protected long createMember(String name, String email, String role) {
        Map<String, Object> request = Map.of(
                "name", name,
                "email", email,
                "password", "password",
    ...
    protected long createStore(String name, long managerId) {
        Map<String, Object> request = Map.of(
                "name", name,
                "managerId", managerId
        );
        ResponseEntity<Map> response = restTemplate.postForEntity("/admin/stores", request, Map.class);

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.CREATED);
        return ((Number) response.getBody().get("id")).longValue();
    }

    protected long createTheme(String name, long storeId) {
        Map<String, Object> request = Map.of(
                "name", name,
                "description", "방탈출",
                "thumbnailUrl", "https://example.com/" + name + ".jpg",
    ...
    protected ResponseEntity<List> findManagedReservations(String token, long storeId) {
        return restTemplate.exchange(
                "/admin/reservations?storeId=" + storeId,
                HttpMethod.GET,
                new HttpEntity<>(authHeaders(token)),
                List.class
        );
    ...
    protected ResponseEntity<Map> findManagedReservationsError(String token, long storeId) {
        return restTemplate.exchange(
                "/admin/reservations?storeId=" + storeId,
                HttpMethod.GET,
                new HttpEntity<>(authHeaders(token)),
    ...
    protected ResponseEntity<Map> findManagedReservationsWithoutAuth(long storeId) {
        return restTemplate.exchange(
                "/admin/reservations?storeId=" + storeId,
                HttpMethod.GET,
                HttpEntity.EMPTY,
    ...
    protected ResponseEntity<Map> changeManagedReservation(
            String token,
            long reservationId,
            LocalDate date,
            long timeId
    ...
    protected ResponseEntity<Void> cancelManagedReservation(String token, long reservationId) {
        return restTemplate.exchange(
                "/admin/reservations/" + reservationId,
                HttpMethod.DELETE,
                new HttpEntity<>(authHeaders(token)),
