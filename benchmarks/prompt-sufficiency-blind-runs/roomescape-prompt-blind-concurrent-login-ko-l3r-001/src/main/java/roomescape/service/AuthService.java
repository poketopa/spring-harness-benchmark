...
public class AuthService {
    ...
    public LoginResponse login(LoginRequest request) {
        ...
        renewActiveSession(member, sessionKey);
        return new LoginResponse(authTokenProvider.createToken(member, sessionKey));
    }

    private void renewActiveSession(Member member, String sessionKey) {
        try {
            member.renewActiveSession(sessionKey);
            memberRepository.saveAndFlush(member);
        } catch (DataIntegrityViolationException exception) {
            throw new RoomescapeException(ErrorCode.UNAUTHORIZED, "로그인 세션을 갱신할 수 없습니다.");
        }
    }
    ...
    public LoginMember authenticate(String accessToken) {
        ...
        if (!member.hasActiveSession(payload.sessionKey())) {
