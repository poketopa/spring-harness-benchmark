...
public class WaitingService {
    ...
    public ReservationResponse create(LoginMember loginMember, ReservationRequest request) {
        ...
        validateCreateAllowed(member, theme, time, request);
        ...
        try {
            ...
            int rank = waitingRepository.countRank(theme, time, request.date(), savedWaiting.getAppliedAt(), savedWaiting.getId());
            return ReservationResponse.from(savedWaiting, rank);
        } catch (DataIntegrityViolationException exception) {
            throw new RoomescapeException(ErrorCode.DUPLICATE_WAITING, "이미 같은 슬롯에 예약 대기 중입니다.");
        }
    }

    private void validateCreateAllowed(Member member, Theme theme, ReservationTime time, ReservationRequest request) {
        if (!reservationRepository.existsByThemeAndTimeAndDate(theme, time, request.date())) {
            throw new RoomescapeException(ErrorCode.RESERVATION_NOT_FOUND, "예약된 슬롯에만 대기할 수 있습니다.");
        }
    ...
    public void cancel(LoginMember loginMember, Long waitingId) {
        ...
        Waiting waiting = waitingRepository.findById(waitingId)
                .filter(target -> target.isOwnedBy(member))
