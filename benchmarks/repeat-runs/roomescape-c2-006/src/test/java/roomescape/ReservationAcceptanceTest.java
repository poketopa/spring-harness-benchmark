...
class ReservationAcceptanceTest extends AcceptanceTestSupport {
    ...
    void memberChangesOwnReservationSchedule() {
        createMember("브라운", "brown@example.com");
        String token = login("brown@example.com");
        long themeId = createTheme("어둠의 방");
        ...
        assertThat(response.getBody().get("timeId")).isEqualTo((int) secondTimeId);

        List<Map<String, Object>> mine = findMine(token);
        assertThat(mine).hasSize(1);
        assertThat(mine.getFirst().get("date")).isEqualTo("2030-05-02");
        assertThat(mine.getFirst().get("timeId")).isEqualTo((int) secondTimeId);
    }

    @Test
    @DisplayName("이미 예약된 슬롯으로 예약을 변경하면 충돌 응답을 반환한다")
    void changeReservationToReservedSlotReturnsConflict() {
        createMember("브라운", "brown@example.com");
        createMember("코니", "cony@example.com");
        String brownToken = login("brown@example.com");
        String conyToken = login("cony@example.com");
        ...
        long reservationId = ((Number) createReservation(brownToken, date, firstTimeId, themeId)
                .getBody().get("id")).longValue();
        createReservation(conyToken, date, secondTimeId, themeId);

        ResponseEntity<Map> response = changeReservation(brownToken, reservationId, date, secondTimeId);
    ...
    void cancelOtherMemberReservationReturnsNotFound() {
        ...
        ResponseEntity<Map> response = restTemplate.exchange(
                ...
                new org.springframework.http.HttpEntity<>(authHeaders(conyToken)),
                Map.class
        );
